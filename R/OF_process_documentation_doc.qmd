...
All of these datasets will feed into the **Outcome Framework (OF) dataset**: `[EAT_Reporting_BSOL].[OF].[OF2_Indicator_SQL_Data]`
...
L3(["Insert data into final table [EAT_Reporting_BSOL].[OF].[OF2_Indicator_SQL_Data] with denominator = NULL "])
L4(["OF dataset"])

M1(["Chapter 1: Building Indicators with Pre-defined Denominator"])
M2(["Example(s): QOF, CVD Prevent, Fingertips"])
M3(["Insert data into final table [EAT_Reporting_BSOL].[OF].[OF2_Indicator_SQL_Data] with denominator populated"])

R1(["Chapter 3: Building Indicators with Derived Denominator"])
R2(["Example(s): NDA"])
R3(["Insert data into final table [EAT_Reporting_BSOL].[OF].[OF2_Indicator_SQL_Data] with denominator populated"])
...
## Update final table {#ch1-sec-update-final-table}

Delete the old indicator data from the final table `[EAT_Reporting_BSOL].[OF].[OF2_Indicator_SQL_Data]` before re-inserting new data to ensure that only the latest data is in the table.
...
## Insert data into final table {#ch2-sec-insert-data-into-final-table}

Ensure to delete the old data from the final table before re-inserting new data.

Union all datasets (i.e., from tables created in [Step 4.7](#ch2-sec-insert-data-into-staging-table), [Step 4.8](#ch2-sec-create-dataset-with-imd-collapsed-to-all), and [Step 4.9](#ch2-sec-create-dataset-with-ethnicity-collapsed-to-all)) and insert them into the final table.
...
INNER JOIN EAT_Reporting_BSOL.Reference.BSOL_ICS_PracticeMapped               AS T2
        ON T1.CURRENT_GP = T2.GPPracticeCode_Original
WHERE T2.ICS_2223 = 'BSOL'
  AND T1.CLEAN_DIABETES_TYPE = 1                      -- Type 1 Diabetes
...
Aggregate the numerator dataset to obtain total numerators across the extracted columns. At this point, you will have a GP-level aggregated dataset (no longer at patient-level), which will later be combined with the denominator dataset before finalising the final table.
...
Aggregate the denominator dataset to obtain total denominators across the extracted columns. At this point, you will have a GP-level aggregated dataset (no longer at patient-level), which will later be combined with the numerator dataset before finalising the final table.
...
If your dataset has ethnicity splits, map each ethnicity to its corresponding code using the reference table below:
...
## Insert data into final table {#ch3-sec-insert-data-into-final-table}

First, delete the old data from the final table before inserting the new data.

Then, insert the data into `[EAT_Reporting_BSOL].[OF].[OF2_Indicator_SQL_Data]`.
